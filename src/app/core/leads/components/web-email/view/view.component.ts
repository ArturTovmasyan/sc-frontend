...
import {FormComponent as WebEmailFormComponent} from '../form/form.component';
...
export class ViewComponent implements OnInit, OnDestroy {
  ...
  constructor(
    ...
    private web_email$: WebEmailService,
      ...
      case 'param_id':
        this.$subscriptions[key] = this.route$.paramMap.subscribe(route_params => {
          if (route_params.has('id')) {
            this.subscribe('get_web_email', {web_email_id: route_params.get('id')});
          }
        });
        break;
      case 'get_web_email':
        this.$subscriptions[key] = this.web_email$.get(params.web_email_id).pipe(first()).subscribe(res => {
          if (res) {
            this.web_email = res;
          }
        });
  ...
  show_modal_edit(name: string): void {
    switch (name) {
      case 'web_email':
        this.web_email$.get(this.web_email.id).subscribe(
          res => {
            this.create_modal(WebEmailFormComponent, data => this.web_email$.edit(data), res);
          },
          error => {
          });
        break;
          ...
          onClick: () => {
            ...
            submit(form_data).subscribe(
              res => {
                ...
                this.subscribe('get_web_email', {web_email_id: this.web_email.id});
  ...
  show(direction: string) {
    switch (direction) {
      case 'previous':
          return this.web_email.previous_email_id === null;
      case 'next':
          return this.web_email.next_email_id === null;
    }
  ...
  getPreviousEmail() {
      this.router$.navigate(['lead', 'web-email', this.web_email.previous_email_id], {queryParams: this.query_params}).then(() => {
          location.reload();
      });
  }

  getNextEmail() {
      this.router$.navigate(['lead', 'web-email', this.web_email.next_email_id], {queryParams: this.query_params}).then(() => {
          location.reload();
      });
  }
}
